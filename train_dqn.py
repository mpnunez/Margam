...
import pyspiel
from utils import get_training_and_viewing_state, add_symmetries, record_episode_statistics, generate_episode_transitions
...
def update_neural_network(step,training_data,agent,target_network,hp,optimizer,mse_loss,writer):
    """
    ...
    non_terminal_states = np.array([trsn.next_state is not None for trsn in training_data])
    resulting_boards = np.array([trsn.next_state if trsn.next_state is not None else np.zeros(trsn.state.shape) for trsn in training_data])
    ...
    if hp["DOUBLE_DQN"]:
        ...
        on_policy_move_mask = tf.one_hot(max_move_inds_on_policy,depth=resulting_board_q_on_policy.shape[1])
    ...
    q_to_train_single_values = rewards + (hp["DISCOUNT_RATE"]**hp["N_TD"]) * np.multiply(non_terminal_states,max_qs)
...
def train_dqn(game_type,hp):
    ...
    for opp in opponents:
        reward_buffer_vs[opp.name] = deque(maxlen=hp["REWARD_BUFFER_SIZE"]//len(opponents))
    ...
    while True:
        ...
        model_save_threshold = max(hp["SAVE_MODEL_ABS_THRESHOLD"], best_reward+hp["SAVE_MODEL_REL_THRESHOLD"])
        if len(reward_buffer) == hp["REWARD_BUFFER_SIZE"] and smoothed_reward > model_save_threshold:
        ...
        for transition in agent_transitions:
            ...
            # Get training data
            training_data = sample_experience_buffer(experience_buffer,hp["BATCH_SIZE"])
            if hp["USE_SYMMETRY"]:
